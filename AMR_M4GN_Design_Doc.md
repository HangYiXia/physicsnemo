...
> **关于 S 的设计修正（M2 实现，尚未实跑测试通过）**：AMR-Transformer 原文把 KH 剪切定义为 `∂u/∂y − ∂v/∂x`，但它**恒等于 −ω**，Router 取绝对值后与涡量完全冗余，4 判据退化为 3。本工作改用**应变率幅值** `S=√(2·S_ij·S_ij)`（速度梯度的对称部分），与 ω（反对称部分）严格独立且恒 ≥ 0，使四个物理先验真正互补。离线 numpy 公式校验（非仓库 torch 代码、非 pytest）：纯剪切流 `u=y` 下 ω=−1、S=1（区分开）；刚体旋转 `u=−y,v=x` 下 ω=2、S=0（应变为零）。**仓库实现的 pytest 验证待补。**
