...
**配套设计文档**：`E:\phys\physicsnemo\docs/design.md`
...
> **文档索引**：总览 `README.md` · 设计 `docs/design.md` · 阶段 M1（本文）〜M7 各 `docs/progress/M*.md`
...
> **为什么手动**：本仓库的 \u4ed3\u5e93\u6839\u76ee\u5f55 目录**没有自带** `download_dataset.sh`（README 里那条 `sh download_dataset.sh` 实际依赖 DeepMind 的脚本）。M1 只看 1 个 case，**只下 `meta.json` + `test.tfrecord` 即可**，train/valid 可暂时不下（M5 全量训练再补）。
...
```bash
cd E:\phys\physicsnemo  REM (repo root)
python -m scripts.visualize_partition --data_dir ./raw_dataset/cylinder_flow/cylinder_flow --split test --case_idx 0
```
...
- **训练 split 的 `graph` 不带 `pos`/`cells`，`edge_attr` 是归一化的**。M2 起需要坐标：按 **方案 P1**（推荐）新建 `data/vortex.py` 子类 `VortexSheddingDatasetAMR`，train 分支补回 `graph.pos`、`graph.gidx`。**这是决策门 D5，M4 开工即定。**
