...
**前置**：M1/M2/M3 已关闭。
**配套设计文档**：`docs/design.md`（§4.3/4.8/4.9、§7.2、§7.4.4~7.4.6、§八 M4）
**工作目录**：`E:\phys\physicsnemo\`（你的运行机：`C:\GitHub\physicsnemo\...`）

> **里程碑总进度**（截至 2026-06-12）：M1 ✅ · M2 ✅ · M3 ✅ · **M4 ✅ 管线跑通** · M5 🟢（AMR>MGN，仅"更大训练集"待算力）· M6 🟡（八组消融就绪，实跑待算力）· M7 🟡（EAGLE reader 就绪，真实数据待验证）
> **文档索引**：总览 `README.md` · 设计 `docs/design.md` · 阶段 M4（本文）+ M1〜M3/M5〜M7 各 `docs/progress/M*.md`
...
├── data/vortex.py                 ✅ 新建：VortexSheddingDatasetAMR（D5/P1：暴露 pos/gidx + get_cache）
...
- **落地**：`data/vortex.py`（mesh 静止，子类额外读一次 `mesh_pos`；`get_cache(gidx)` 懒加载分区缓存）。
...
- **做什么**：确认下列文件都是 M4 最新版（注意三个脚本在 \u4ed3\u5e93\u6839 根目录，不在 `amr_m4gn/`）：
...
- **做什么**（**用 `train` split**，见下「为什么」）：
  ```bash
  python -m scripts.train_amr_m4gn --data_dir ./raw_dataset/cylinder_flow/cylinder_flow --split train --case_idx 0 --num_steps 50 --epochs 300 --omega_thresh 30
  ```
...
| §7.6 数据/训练入口（P1 子类、NMSE）| `data/vortex.py` + `train_amr_m4gn.py` + `conf/config_amr_m4gn.yaml` |
