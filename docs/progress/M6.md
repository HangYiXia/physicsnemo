...
**配套文档**：设计 `docs/design.md`（§6.5 消融计划 / §八 M6）；阶段 `docs/progress/M1.md`〜`M5.md`

> **里程碑总进度**：M1 ✅ · M2 ✅ · M3 ✅ · M4 ✅ · M5 🟢 · **M6 🟡（八组消融代码+脚本+单测就绪，实跑待算力）** · M7 🟡（EAGLE reader 就绪，真实数据待验证）
>
> **文档索引**：总览 `README.md` · 设计 `docs/design.md` · 阶段 M6（本文）+ M1〜M5/M7 各 `docs/progress/M*.md`
>
...
| 3 | δ重叠 + 虚拟步接线 | `macro_transformer.py`（halo 池化/分发）+ `model.py` + `data/vortex.py`（`x_prev`）| 🟡 已实现 |
...
> 在 `` 下执行。前置：M5 的 modal 缓存已建；w/o Modal 行额外需 `_nomodal` 缓存。

```bash
# (run from repo root)
# ── 0) 额外预处理：几何-only 缓存（供 w/o Modal 行）──────────────────────────
python -m scripts.preprocess_partitions --data_dir ./raw_dataset/cylinder_flow/cylinder_flow --split train --num_cases 20 --out_dir ./amr_cache --no_modal
python -m scripts.preprocess_partitions --data_dir ./raw_dataset/cylinder_flow/cylinder_flow --split test  --num_cases 1  --out_dir ./amr_cache --no_modal

# ── 1) 跑全部 8 组消融（训练 + 同 rollout 评估 + 出表/图）────────────────────
python -m scripts.run_ablation --data_dir ./raw_dataset/cylinder_flow/cylinder_flow --cache_dir ./amr_cache --num_cases 20 --num_steps 100 --batch_size 2 --epochs 200 --noise_std 0.02 --omega_thresh 8.9 --test_case 0 --rollout 80
...
python -m scripts.preprocess_partitions --data_dir ./raw_dataset/cylinder_flow/cylinder_flow --split test --num_cases 10 --out_dir ./amr_cache
python -m scripts.eval_rollout --data_dir ./raw_dataset/cylinder_flow/cylinder_flow --cache_dir ./amr_cache --amr_ckpt ./checkpoints_amr/amr_m4gn_epoch199.pt --mgn_ckpt ./checkpoints_mgn/mgn_epoch199.pt --num_cases 10 --num_steps 90 --rollout 80 --omega_thresh 8.9

# ── (可选) 只跑某几组 ──────────────────────────────────────────────────────
python -m scripts.run_ablation --data_dir ./raw_dataset/cylinder_flow/cylinder_flow --cache_dir ./amr_cache --num_cases 20 --num_steps 100 --epochs 200 --noise_std 0.02 --omega_thresh 8.9 --only full "w/ overlap" "w/ virtual"

# ── (可选) 单独训练某个消融配置（产 checkpoint，便于后续可视化）─────────────
python -m scripts.train_amr_m4gn_full --data_dir ./raw_dataset/cylinder_flow/cylinder_flow --cache_dir ./amr_cache --num_cases 20 --num_steps 100 --batch_size 2 --epochs 200 --noise_std 0.02 --omega_thresh 8.9 --use_overlap --tag amr_overlap
...
| `data/vortex.py` | + `graph.x_prev`（前一帧归一化速度）| 供虚拟步路由 |
...
| `--clip` | 1.0 | 梯度范数裁剪（与 `train_amr_m4gn_full.py` 同默认,**勿关**——8 组消融共用此防护,见 M5 §9 失败案例）|
| `--warmup` | 5 | 线性 lr 暖启动轮数（同上）|
