...
**配套文档**：设计 `docs/design.md`（§八 M7 / 决策门 D9）；总览 `README.md`
...
| # | 工作 | 交付物 | 状态 |
| --- | --- | --- | --- |
| 1 | EAGLE 数据 reader | `data/eagle.py`（`load_eagle_case` + `EagleDatasetAMR`）| 🟡 按官方格式写，合成数据验过管线，真实数据待验证 |
| 2 | 预处理并行化（U6）| `preprocess_partitions.py`: `--workers` + `--source eagle` | 🟡 已实现 |
| 3 | 数据集切换 | `data.vortex.make_amr_dataset` + 训练/推理/评估加 `--dataset` | 🟡 已实现 |
| 4 | 单元测试 | `tests/test_data/eagle.py`（合成 npz 验形状/格式）| 🟡 已写，待目标机 `pytest` |
...
> **诚实边界**：键名已按官方页核对，但 **triangles 单独文件的确切布局**与 **node_type id 语义**仍需用真实文件核对（见 §8）。`data/eagle.py` 顶部「ADAPTER」段集中了所有需核对处。
...
>
> **前置**：把 EAGLE `.npz` 放到 `--data_dir`，每个 sim 一个文件；triangles 单独文件放好（让 `EAGLE_TRIANGLE_FILES` 能找到，或先用 Delaunay 兜底）；可选 `{split}.txt` 列出 id（否则按 `*.npz` 排序）。先到 `data/eagle.py` 顶部核对 `EAGLE_KEYS` / triangles 路径 / 节点类型映射。

```bash
# (run from repo root)
# 0) 并行预处理 EAGLE（U6；--workers 按 CPU 核数设）
python -m scripts.preprocess_partitions --source eagle --data_dir <EAGLE_DIR> --split train --num_cases 50 --out_dir ./eagle_cache --workers 8
python -m scripts.preprocess_partitions --source eagle --data_dir <EAGLE_DIR> --split test  --num_cases 10 --out_dir ./eagle_cache --workers 8

# 1) 训练（注意 EAGLE 网格更大，可调小 batch / processor_size，或减 num_cases）
python -m scripts.train_amr_m4gn_full --dataset eagle --data_dir <EAGLE_DIR> --cache_dir ./eagle_cache --num_cases 50 --num_steps 100 --batch_size 1 --epochs 100 --noise_std 0.02 --omega_thresh 8.9 --tag amr_eagle

# 2) 多 case 平均评估（如有 MGN-EAGLE 基线则带 --mgn_ckpt）
python -m scripts.eval_rollout --dataset eagle --data_dir <EAGLE_DIR> --cache_dir ./eagle_cache --amr_ckpt ./checkpoints_amr/amr_eagle_epoch99.pt --num_cases 10 --num_steps 90 --rollout 80 --omega_thresh 8.9

# 3) 场 GIF
python -m scripts.inference_amr_m4gn --dataset eagle --data_dir <EAGLE_DIR> --cache_dir ./eagle_cache --ckpt ./checkpoints_amr/amr_eagle_epoch99.pt --split test --case_idx 0 --num_steps 90 --rollout 80 --omega_thresh 8.9 --gif

# (现在就能跑) 合成 npz 的 reader 单测
pytest tests/test_data/eagle.py -v
```
...
| reader 单测 | `pytest tests/test_data/eagle.py -v` | 通过数（应 2/2）|
...
**最重要**：先回传一个 EAGLE `.npz` 的 `keys` 与各数组 `shape`、以及 `np.unique(mask)`，本人据此校正 `data/eagle.py` 适配层，再推进训练/对比。之后按 §4 表回传，本人原样录入、不改数字。
