...
**前置**：M1 已由你实跑 4 个 case 验证关闭（环境、数据集下载、`visualize_partition.py` 基础用法见 `docs/progress/M1.md`，本文不重复）
**配套设计文档**：`docs/design.md`（§4.6 / §7.4.2 / §八 M2）
...
> **文档索引**：总览 `README.md` · 设计 `docs/design.md` · 阶段 M2（本文）+ M1/M3〜M7 各 `docs/progress/M*.md`
...
```bash
python -m scripts.visualize_partition --data_dir ./raw_dataset/cylinder_flow/cylinder_flow --split test --case_idx 0  --timestep 300 --plot_physics
python -m scripts.visualize_partition --data_dir ./raw_dataset/cylinder_flow/cylinder_flow --split test --case_idx 1  --timestep 300 --plot_physics
python -m scripts.visualize_partition --data_dir ./raw_dataset/cylinder_flow/cylinder_flow --split test --case_idx 50 --timestep 300 --plot_physics
python -m scripts.visualize_partition --data_dir ./raw_dataset/cylinder_flow/cylinder_flow --split test --case_idx 99 --timestep 300 --plot_physics
```
...
![M2 物理量场图 case0 t300](partition_vis/case0/07_physics_fields.png)
