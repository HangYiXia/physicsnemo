...
**配套文档**：设计 `docs/design.md`；阶段 `docs/progress/M1.md`〜`M4.md`

> **里程碑总进度**：M1 ✅ · M2 ✅ · M3 ✅ · M4 ✅ · **M5 🟢（小验证档全部完成，仅"更大训练集"待算力）** · M6 🟡（八组消融就绪，实跑待算力）· M7 🟡（EAGLE reader 就绪，真实数据待验证）
> **文档索引**：总览 `README.md` · 设计 `docs/design.md` · 阶段 M5（本文）+ M1〜M4/M6/M7 各 `docs/progress/M*.md`
...
> 全部命令在 `` 下执行；均为单行。阈值 `omega_thresh 8.9` 为本数据集标定值（见 §4.4）。

```bash
# (run from repo root)
# ── 0) 预处理：train 20 case + test 1 case（几何缓存，只跑一次）─────────────
python -m scripts.preprocess_partitions --data_dir ./raw_dataset/cylinder_flow/cylinder_flow --split train --num_cases 20 --out_dir ./amr_cache
...
python -m scripts.calibrate_thresholds --data_dir ./raw_dataset/cylinder_flow/cylinder_flow --cache_dir ./amr_cache --num_cases 4 --num_steps 50 --stride 5

# ── 2) 训练 AMR-M4GN 与 MGN（完全同预算）────────────────────────────────────
python -m scripts.train_amr_m4gn_full --data_dir ./raw_dataset/cylinder_flow/cylinder_flow --cache_dir ./amr_cache --num_cases 20 --num_steps 100 --batch_size 2 --epochs 200 --noise_std 0.02 --omega_thresh 8.9
python -m scripts.train_mgn_baseline  --data_dir ./raw_dataset/cylinder_flow/cylinder_flow --num_cases 20 --num_steps 100 --batch_size 2 --epochs 200 --noise_std 0.02

# ── 3) 可视化：单帧面板 / rollout 曲线 / 场 GIF ────────────────────────────
# 3a 单帧 9 宫格 (pred vs GT vs |err|) → 09_prediction_*.png
python -m scripts.inference_amr_m4gn --data_dir ./raw_dataset/cylinder_flow/cylinder_flow --cache_dir ./amr_cache --ckpt ./checkpoints_amr/amr_m4gn_epoch199.pt --split test --case_idx 0 --timestep 25 --num_steps 90 --omega_thresh 8.9
# 3b rollout 误差曲线 + 速度/压强场 GIF → 10_rollout_*.png + animations/amr_m4gn_case0_{u,v,p}.gif
python -m scripts.inference_amr_m4gn --data_dir ./raw_dataset/cylinder_flow/cylinder_flow --cache_dir ./amr_cache --ckpt ./checkpoints_amr/amr_m4gn_epoch199.pt --split test --case_idx 0 --num_steps 90 --rollout 80 --omega_thresh 8.9 --gif --gif_fields u v p

# ── 4) AMR vs MGN 对比曲线 → 12_compare_rollout_case0.png ──────────────────
python -m scripts.compare_baselines --data_dir ./raw_dataset/cylinder_flow/cylinder_flow --cache_dir ./amr_cache --amr_ckpt ./checkpoints_amr/amr_m4gn_epoch199.pt --mgn_ckpt ./checkpoints_mgn/mgn_epoch199.pt --split test --case_idx 0 --num_steps 90 --rollout 80 --omega_thresh 8.9
# 4b 加 --gif：AMR/MGN/GT 三行场动画 → animations/compare_case0_{u,v,p}.gif
python -m scripts.compare_baselines --data_dir ./raw_dataset/cylinder_flow/cylinder_flow --cache_dir ./amr_cache --amr_ckpt ./checkpoints_amr/amr_m4gn_epoch199.pt --mgn_ckpt ./checkpoints_mgn/mgn_epoch199.pt --split test --case_idx 0 --num_steps 90 --rollout 80 --omega_thresh 8.9 --gif --gif_fields u v p
...
```
  ...
  VortexSheddingDataset                          preprocess_partitions.py ─写─► amr_cache/partition_cache_*.pt
   (physicsnemo 内置)                              用到 modal_decomp/segmentation/pe/amr_router(build_l1_to_l0)
          │                                             │
          ▼                                             ▼
   train.py / inference.py                       data.vortex.VortexSheddingDatasetAMR
...
| `--clip` | 1.0 | 梯度范数裁剪上限（0 关）。**默认必开**：实测无 clip 时 batched 训练在 epoch 5~10 会突发发散（NMSE 跳到 ~1，输出塌缩到 0）。详见 §10 失败案例。|
| `--warmup` | 5 | 线性 lr 暖启动轮数（lr/10 → lr）。配合 clip 进一步抑制早期梯度尖峰。|
| `--device` | 自动 | `cuda`/`cpu` |
...
| `--ckpt_every` | 20 | 存档间隔 |
| `--clip` | 1.0 | 梯度范数裁剪上限（同 AMR，公平起见）|
| `--warmup` | 5 | 线性 lr 暖启动轮数（同 AMR）|
| `--device` | 自动 | 设备 |
...
## 9. 失败案例与修复（2026-06-12 训练发散事件）

**现象**：用户在目标机用以下命令跑全档训练:

```bash
python -m scripts.train_amr_m4gn_full --data_dir ./raw_dataset/cylinder_flow/cylinder_flow \
...
日志:
```
epoch    0  NMSE 7.91e-01
epoch    5  NMSE 5.43e-01     ← 还在下降
epoch   10  NMSE 9.97e-01     ← 跳到 ~1.0
epoch   15  NMSE 9.97e-01     ← 卡死
epoch   30  NMSE 9.97e-01
```

**诊断**：NMSE ≈ 1.0 = 输出塌缩到 0。理由:NMSE = mean((pred-y)²) / mean(y² + eps),pred ≈ 0 时分子分母约等,商 ≈ 1。说明权重在 epoch 5~10 之间被某 batch 的极端梯度推进死区(典型 dead ReLU 或权重爆炸后梯度归零)。

**根因**:`train_amr_m4gn_full.py` 与 `train_mgn_baseline.py` 的初始版**没有梯度裁剪和 NaN 守护**。M4 单 case overfit (`train_amr_m4gn.py`) 在 batch_size=1 时未触发,M5 batched 训练梯度方差更大才暴露。

**修复**(2026-06-12 提交):

1. **梯度范数裁剪** `torch.nn.utils.clip_grad_norm_(params, max_norm=args.clip)`,默认 `--clip 1.0`(可 0 关)。
2. **线性 LR 暖启动**:前 `--warmup 5` 轮 lr 从 base_lr/10 线性升到 base_lr,跨过早期梯度尖峰最危险的几个 epoch。warmup 期间不调用 `sched.step()`。
3. **NaN/inf 守护**:`if not torch.isfinite(loss): nan_skipped += 1; continue`。一个 NaN batch 不会污染权重。
4. **诊断日志**:每 5 epoch 打印 `grad_norm`(本轮 clip 前的均值)和 `nan_skipped`。
5. AMR 与 MGN baseline **同步加同样默认值**,保证对比公平。

**预期健康日志样式**:
```
epoch    0  NMSE 7.91e-01  lr 2.80e-04  grad_norm 0.50    ← warmup 中
epoch    5  NMSE ~5e-01    lr 1.00e-03  grad_norm 0.7-0.9 ← warmup 完
epoch   10  NMSE ~3-4e-01  lr 1.00e-03  grad_norm 0.4-0.7 ← 继续降
epoch  100  NMSE ~1-2e-02  lr ~9.99e-04 grad_norm <0.5
```

**验证策略**(若仍发散):依次降级排查
1. `--lr 5e-4`(降一半)
2. `--clip 0.5`(裁更狠)
3. `--noise_std 0.0`(去噪声排查物理量数值稳定性)
4. `--omega_thresh inf`(关 AMR 路由,先验证 GNN+Transformer 主路无 bug)
5. `--warmup 10`(更长 warmup)

**后续记录**:用户重跑后请把 `grad_norm`/`nan_skipped` 与正常 NMSE 曲线粘进 §10 回传区。
