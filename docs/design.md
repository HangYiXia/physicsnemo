...
> **文档索引**：上手总览 `README.md`（先读这个）；本设计文档 `docs/design.md`；阶段手册 `docs/progress/M1.md`〜`M7.md`（各含「拿到代码后每步做什么/为什么/应得什么」的操作说明）。
...
- **训练时**：阈值在预定义区间均匀随机采样。原文（归一化尺度）`G:[0.1,2.0], ω:[0.2,4.0], M:[0.5,10.0], S:[0.2,4.0]`；**本数据物理尺度需标定**——M5 step 5 已标定 **ω:[2.83, 25.8]**（物理 |ω|，见 §八 D3），G/M/S 待同法标定。模型见过各种粒度的 Token 化，泛化性强。
...
| `data/vortex.py`（子类）| ✅ 实现（D5/P1，overfit 实跑）| 同 VortexSheddingDataset | graph(+pos,+gidx)+get_cache | 见 §7.2-A |
...
- **CLI**：`python -m scripts.preprocess_partitions --data_dir ... --split train --K0 64 --K1 256 --num_cases 400`。
...
- **`data/vortex.py`**：`VortexSheddingDatasetAMR(VortexSheddingDataset)`，train 分支额外返回 `graph.pos`、`graph.gidx`；并在 `__init__` 按需加载/触发 `preprocess_partitions`（缺缓存则报错提示先跑预处理）。
...
- **退出标准**：解析场单测误差 < 5%；可视化中圆柱后方涡街 ω 显著高于来流区。**实测结果**：pytest 8/8 通过；t=300 四 case `|ω|p99=68–208` 同数量级，ω 图脱涡涡街清晰、来流≈0。详见 `docs/progress/M2.md` §七/§十。
...
- **退出标准**：单测四例全过 + 可视化尾迹保细/来流合并。**实测结果**：`amr_router` 单测 6/6、`pe` 单测 5/5（共 11/11）通过；4 case×t=300（ω 单通道 p70）T=129–135、reduction ~48%，`08_routing.png` 红（保细）集中于尾迹+底壁、蓝（合并）覆盖来流。详见 `docs/progress/M3.md`。
- **🚩 决策门 D3（K0/K1 取值）**：✅ **已标定（M5 step 5，`calibrate_thresholds.py`）**：真实 per-seg |ω| 量级 p50≈7.6/p90≈33/p99≈108（比原文归一化区间高 1~2 量级）；绝对 ω 阈值≈8.9 时 mean T≈164（[K0,K1] 中段），**K0=64/K1=256 确认合适**；训练期 ω 采样区间定为 **[2.83, 25.8]**（已写入 `DEFAULT_RANGES`）；同一绝对阈值下 T 随帧波动，**「T 随物理变化」闭环**。G/M/S 待同法标定。

### M4 — 端到端跑通（3 天）🟢 管线跑通（overfit 收敛度待调参）

- **目标**：完整模型在**单个 case** 上 overfit 成功。
- **新增**：`micro_gnn.py`、`macro_transformer.py`、`model.py`、`data/vortex.py`、`preprocess_partitions.py`、`train_amr_m4gn.py`、`conf/config_amr_m4gn.yaml`。
- **配合数据**：1 个 case 全时间步 + 在线/离线 `partition_cache`。
- **退出标准**：(1) 集成测试无 NaN、全参有梯度；(2) overfit 单 case：loss 单调降至接近 0。**实测结果**：组件单测 9/9 + 集成测试 3/3 通过；overfit 单 case NMSE **0.92→0.013（降约 70×、趋势向下）→ 端到端管线正确且可学习**；但后期在 1e-2~6e-2 震荡、未压到 ≈0（49 帧同拟合 + lr 未精调，属调参问题，非管线缺陷）。详见 `docs/progress/M4.md`。
...
- **状态 🟡（代码就绪，实跑待算力）**：开关/脚本/单测全部完成，§6.5 **八行全部可跑**（δ重叠、虚拟步已接线，因基线本就 δ=0/无虚拟步，实现为「w/ overlap」「w/ virtual」度量增益）；**未实跑**（本机无算力）。另附 `eval_rollout.py` 多 test case 平均评估补 M5 单 case 短板。详见 `docs/progress/M6.md`。

### M7 — EAGLE 大规模扩展（可选，5 天）

- **目标**：大规模湍流能力验证。
- **新增**：EAGLE 数据 reader `data/eagle.py`（`load_eagle_case` + `EagleDatasetAMR`，复用基类静态方法、图格式一致）；预处理并行化 `--workers`（U6）+ `--source eagle`；`make_amr_dataset` 工厂 + 训练/推理/评估 `--dataset` 开关。
...
- **状态 🟡（代码就绪，真实数据待验证）**：reader/并行/切换/合成数据单测全部完成；按 EAGLE **公开格式**实现，已用**合成 npz** 验证形状/图格式管线，但**未在真实 EAGLE 上跑过**（字段名/节点类型 id/动态网格/显存待核对）。动态网格按「sim 内 t=0 连通性固定」处理（假设 A1）。详见 `docs/progress/M7.md`。
