...
本目录是 NVIDIA PhysicsNeMo `vortex_shedding_mgn` 示例的扩展。原版 MGN 脚本（`train.py`/`inference.py`）保持不动；AMR-M4GN 的全部代码、脚本、文档都新增在此目录。
...
| 你想干嘛 | 跑哪个脚本 | 看什么 |
| --- | --- | --- |
| 训练 AMR-M4GN | `train_amr_m4gn_full.py` | 终端 NMSE 下降 + `checkpoints_amr/*.pt` |
| 训练对照基线 MGN | `train_mgn_baseline.py` | 同上，`checkpoints_mgn/*.pt` |
| **看预测场动画（GIF）** | `inference_amr_m4gn.py --gif` | `animations/amr_m4gn_case0_{u,v,p}.gif` |
| **AMR vs MGN vs 真值** | `compare_baselines.py --gif` | `animations/compare_case0_*.gif` + 误差曲线 |
...
cd raw_dataset && sh download_dataset.sh cylinder_flow && cd ..
```
...
python train_amr_m4gn_full.py --data_dir ./raw_dataset/cylinder_flow/cylinder_flow --cache_dir ./amr_cache --num_cases 4 --num_steps 50 --batch_size 2 --epochs 50 --noise_std 0.02 --omega_thresh 8.9

# (3) 看效果：rollout 误差曲线 + 速度/压强场 GIF
python inference_amr_m4gn.py --data_dir ./raw_dataset/cylinder_flow/cylinder_flow --cache_dir ./amr_cache --ckpt ./checkpoints_amr/amr_m4gn_epoch49.pt --split test --case_idx 0 --num_steps 50 --rollout 40 --omega_thresh 8.9 --gif --gif_fields u v p
```
...
正式档（20 算例 × 100 步 × 200 轮）+ 公平对比。完整命令序列见
**`AMR_M4GN_Progress_M5.md` §2「一键复现」**。核心四步：

```bash
# 预处理 train 20 + test 1（同 §3 但 --num_cases 20）
# 训练 AMR-M4GN 与 MGN（同预算）
python train_amr_m4gn_full.py ... --num_cases 20 --num_steps 100 --epochs 200 --omega_thresh 8.9
python train_mgn_baseline.py  ... --num_cases 20 --num_steps 100 --epochs 200
# AMR vs MGN vs 真值：误差曲线 + 三行场动画
python compare_baselines.py ... --amr_ckpt ./checkpoints_amr/amr_m4gn_epoch199.pt --mgn_ckpt ./checkpoints_mgn/mgn_epoch199.pt --rollout 80 --omega_thresh 8.9 --gif
# 多 test case 平均（更严谨）
python eval_rollout.py ... --amr_ckpt ... --mgn_ckpt ... --num_cases 10 --rollout 80
```
...
├── train_amr_m4gn_full.py       # 训练 AMR-M4GN
├── train_mgn_baseline.py        # 训练 MGN 基线（同预算）
...
└── README_AMR_M4GN.md           # 本文
```

每个脚本的**逐参数含义**见 `AMR_M4GN_Progress_M5.md` §7。
